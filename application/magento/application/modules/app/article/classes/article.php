...
abstract class Article extends \vendor\Module
{
    ...
    public function get_last_articles($limit = 10)
    {
        $article_model = self::get_model(MY_MODEL_NAME_DB_ARTICLES);
        $condition = '';
        $order = 'id DESC';
    ...
    {
        $article_model = self::get_model(MY_MODEL_NAME_DB_ARTICLES);
        $language = components\Language::get_instance()->get_language();
        ...
        $sql = "SELECT
                ...
                ORDER by cn.name";
        return \vendor\DBase_Mysql::model()->get_connect()->query($sql, \PDO::FETCH_ASSOC)->fetchAll();
    }
