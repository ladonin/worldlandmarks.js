...
abstract class Map extends \vendor\Module
{
    ...
    const GEOCODE_SERVICE_URL = 'https://geocode-maps.yandex.ru/1.x/?';
    ...
    {
        $map_module = self::get_module(MY_MODULE_NAME_MAP);
        $result = $map_module->get_point_content_by_ids(array($id), true, null, self::get_module(MY_MODULE_NAME_SERVICE)->is_show_relevant_placemarks(), self::get_module(MY_MODULE_NAME_SERVICE)->is_show_another_placemarks());
        return $result[$id];
    }
    /*
...
// если не пустой то подготавливаеи
    ...
    public function get_adress_by_coords($coords, $language = '')
    {
        ...
        if (my_is_empty(@$language)) {
            self::concrete_error(array(MY_ERROR_LANGUAGE_NOT_FOUND, 'language:' . @$language));
        }
        $query = self::GEOCODE_SERVICE_URL . 'format=json&apikey='.$access_settings['ya_apikey'] . '&geocode='.$coords['x'] . ',' . $coords['y'].'&lang='.$language;

        $request_json = file_get_contents($query);
        $request_array = json_decode($request_json, true);
        $adress = array();
// если гугл не выдал то, что нам нужно
        if (!isset($request_array['response'])) {
            $adress['address_components'] = array();
            $adress['formatted_address'] = '[ERROR]: api servise answer is wrong';
        } else {
// если работает
            if (my_array_is_empty(@$request_array['response'])) {
                $adress['address_components'] = array();
                $adress['formatted_address'] = 'undefined place';
            } else {
                $adress['address_components'] = $request_array['response']['GeoObjectCollection']['featureMember'][0];
                $adress['formatted_address'] = $request_array['response']['GeoObjectCollection']['featureMember'][0]['GeoObject']['metaDataProperty']['GeocoderMetaData']['text'];
            }
        }
...
JOIN landmarks_map_photos on phh2.phh2_id=landmarks_map_photos.id
    ...
    public function get_point_content_by_ids(array $ids = null, $need_plain_text = true, $order = null, $need_relevant = false, $need_another = false)
    {
        if (!$ids) {
            return array();
        }
        if (my_array_is_not_empty($ids)) {
...
// делаем массив безопасным
            ...
            $result = $this->prepare_result($result, $need_relevant, $need_another);
